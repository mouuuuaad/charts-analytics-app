        ...
        if (!isPremium) {
            ...
            if (attempts >= MAX_FREE_ATTEMPTS) {
                toast({
                    ...
                    description: ( <div className="flex flex-col gap-1.5"> <p>Used all {MAX_FREE_ATTEMPTS} free attempts. Upgrade for unlimited.</p> 
      ...
      if (typeof window !== 'undefined') {
        ...
        if (!isPremium) {
            ...
            localStorage.setItem('analysisAttempts', attempts.toString());
             if (attempts === MAX_FREE_ATTEMPTS) toast({ title: "Last Free Attempt", description: "Upgrade for unlimited access.", action: (<Button size="sm" asChild className="h-7 text-xs"><Link href="/profile" legacyBehavior passHref><a>Upgrade</a></Link></Button>), duration: 7000 });
