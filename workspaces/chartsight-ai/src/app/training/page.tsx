  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      ...
      if (generatedQuestions && generatedQuestions.length > 0) {
        ...
        setFeedback(null); setIsCorrect(null); setScore(0); setUserLevel('Not Assessed');
        ...
        <AnimatePresence mode="wait">
          <motion.div key={currentQuestion.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.15 }}>
            <CardContent className="pt-3 p-2 md:p-3 space-y-3">
              <p className="text-md font-medium leading-normal"> {currentQuestion.questionText} </p>
              ...
              {quizStatus === 'feedback_shown' && feedback && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} transition={{ duration: 0.15 }}
                  className={`p-2 rounded border text-xs ${ isCorrect ? 'border-foreground bg-background' : 'border-destructive bg-destructive/10 text-destructive' }`}
                >
